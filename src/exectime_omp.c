...
int main(int argc, char *argv[]) {
  ...
  case 1: // COO 1D
  {
    ...
    for (i = 0; i < OUTER_MAX; i++) {
      for (int k = 0; k < 100; k++) {
        spmv_coo_omp(row, col, coo_val, nz, N, x, y, y_local);
      }
      for (int t = 0; t < num_threads; t++) {
        zero_arr(N, y_local[t]);
      }
      zero_arr(N, y);
  ...
  case 2: // CSR
  {
    ...
    for (i = 0; i < OUTER_MAX; i++) {
      for (int k = 0; k < 100; k++) {
        spmv_csr_omp(row_ptr, colind, val, N, x, y);
      }
      zero_arr(N, y);
    ...
    // paralellize at outerloop with parallel
  case 3: // DIA data 1-dim
  {
    ...
    for (i = 0; i < OUTER_MAX; i++) {
      for (int k = 0; k < 100; k++) {
        spmv_dia_omp(offset, dia_data, N, num_diags, stride, x, y);
      }
      zero_arr(N, y);
  ...
  case 4: // ELL format
  {
    ...
    for (i = 0; i < OUTER_MAX; i++) {
      for (int k = 0; k < 100; k++) {
        spmv_ell_omp(indices, ell_data, N, num_cols, x, y);
      }
      zero_arr(N, y);
  ...
  default:
    printf("Wrong format. Must be coo,csr,dia,ell\n");
    exit(1);
    break;
  }
